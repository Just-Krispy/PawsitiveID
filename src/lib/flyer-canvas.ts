...
export async function renderFlyerToCanvas(
  ...
  ctx.fillText(
    "Do you recognize this dog? Please help us find their owner!",
    W / 2,
    130
  );
  ...
  if (!contactName.trim() && !contactPhone.trim() && !contactEmail.trim()) {
    ...
    ctx.fillText(
      "(Add your contact info above before downloading)",
      W / 2,
      contactY
    );
  }
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    "Generated by PawsitiveID  •  pawsitiveid.vercel.app",
    W / 2,
    footerY + 32
  );
...
export async function generateFlyerBlob(
  options: FlyerRenderOptions
): Promise<Blob | null> {
  const canvas = document.createElement("canvas");
  await renderFlyerToCanvas(canvas, options);
